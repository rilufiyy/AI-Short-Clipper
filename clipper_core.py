...
class AutoClipperCore:
    ...
    def __init__(
        ...
        # Source tracking (set by process() / find_highlights_only())
        self.source_url = ""
        ...
        # Drive session folder ID created once per video batch (reused per clip)
        self.gdrive_session_folder_id = None
...
{transcript}"""
    ...
    def get_continuation_prompt():
        """System prompt for 'continue_part' mode — sequential connected clips."""
        return """Kamu adalah EDITOR SHORT-FORM TIER A untuk konten PODCAST viral (TikTok / Reels / Shorts).

OUTPUT ANDA AKAN LANGSUNG DIGUNAKAN UNTUK PRODUKSI.
Kesalahan durasi, urutan, atau format = GAGAL TOTAL.

==================================================
TUGAS UTAMA: MODE CONTINUE PART (NON-NEGOTIABLE)
=================================================

Ini adalah mode CONTINUE PART VIDEO — BUKAN mode random viral.

CARA KERJA MODE INI:
1. Temukan SATU alur diskusi/cerita terbaik dan paling menarik dalam video.
2. Potong alur tersebut menjadi TEPAT {num_clips} bagian yang BERURUTAN dan SALING TERHUBUNG.
3. Setiap bagian harus terasa seperti kelanjutan langsung dari bagian sebelumnya.
4. Audiens harus merasakan "ada kelanjutannya!" setelah menonton setiap bagian.

ATURAN MUTLAK:
* Clips HARUS berurutan secara waktu: start_time clip N < start_time clip N+1
* SEMUA clips berasal dari SATU topik/alur yang sama — bukan dari topik berbeda
* Gap antara end_time clip N dan start_time clip N+1 MAKSIMAL 60 detik
* DILARANG melompat ke bagian video yang tidak berhubungan

==================================================
PRINSIP PEMILIHAN ALUR (WAJIB)
================================

Pilih alur/cerita yang memiliki ARC NARATIF lengkap:

1. PROGRESSION: setiap bagian membangun intensity lebih tinggi
2. CLIFFHANGER NATURAL: setiap bagian berakhir saat audiens masih ingin tahu
3. KONEKSI LOGIS: bagian selanjutnya menjawab atau melanjutkan pertanyaan dari bagian sebelumnya

Struktur ideal untuk {num_clips} bagian:
- Bagian 1: HOOK + Perkenalan konflik/cerita utama
- Bagian 2 dst: Buildup + detail yang semakin intens / mengejutkan
- Bagian terakhir: Puncak / resolusi / reveal mengejutkan

==================================================
FORMAT JUDUL (WAJIB DIIKUTI)
============================

Gunakan format: "Part [N]: [Judul Menarik]"

Contoh untuk 3 bagian:
- "Part 1: Saat Bisnisnya Hampir Bangkrut"
- "Part 2: Semua Tabungan Habis dalam 3 Bulan"
- "Part 3: Keputusan yang Mengubah Segalanya"

Judul HARUS terasa seperti seri yang mengalir alami satu sama lain.
...
Jika durasi < 60 detik → PERPANJANG dengan konteks sebelum/sesudah dalam alur yang sama.
Jika durasi > 120 detik → Pangkas filler TANPA merusak alur cerita.
...
1. Jumlah segment = {num_clips} ?
2. Semua start_time berurutan (clip 1 < clip 2 < clip 3 dst) ?
3. Gap antara clips tidak lebih dari 60 detik ?
4. Semua clips dari satu alur/topik yang sama ?
5. Semua durasi 60–120 detik ?
6. Semua punya tepat 6 field ?
7. virality_score berupa integer 1–10 ?
...
1. "start_time" (string) → Format: "HH:MM:SS,mmm"
2. "end_time" (string) → Format: "HH:MM:SS,mmm"
3. "title" (string) → Format "Part N: [judul]", maks 60 karakter
4. "description" (string) → Maks 150 karakter, jelaskan isi bagian ini
5. "virality_score" (integer) → 1–10 (HARUS ANGKA, BUKAN STRING)
6. "hook_text" (string) → Kalimat hook kuat, maks 15 kata, TANPA emoji
...
[{{"start_time":"HH:MM:SS,mmm","end_time":"HH:MM:SS,mmm","title":"Part 1: ...","description":"...","virality_score":8,"hook_text":"..."}}]
...
{transcript}"""
    ...
    def process(self, url: str, num_clips: int = 5, add_captions: bool = True, add_hook: bool = True):
        ...
        # Extract transcript text for each highlight (for enriched output)
        for h in highlights:
            if "transcript_text" not in h:
    ...
    def find_highlights_with_transcription(self, video_path: str, video_info: dict,
        ...
        highlights = self.find_highlights(transcript, video_info, num_clips, clip_mode=clip_mode)
    ...
    def find_highlights(self, transcript: str, video_info: dict, num_clips: int,
                        clip_mode: str = "random") -> list:
        """Find highlights using AI (OpenAI-compatible API)

        Args:
            clip_mode: "random" for independent viral clips, "continue_part" for sequential parts
        """
        mode_label = "Continue Part" if clip_mode == "continue_part" else "Random"
        self.log(f"[2/4] Finding highlights (using {self.model}, mode: {mode_label})...")

        # In continue_part mode request exactly num_clips; in random mode request extras to filter
        request_clips = num_clips if clip_mode == "continue_part" else num_clips + 3
...
- Deskripsi: {video_info.get('description', '')[:500]}"""
        ...
        # Choose prompt based on clip mode
        if clip_mode == "continue_part":
    ...
    def process_clip(self, video_path: str, highlight: dict, index: int, total_clips: int = 1, add_captions: bool = True, add_hook: bool = True, pre_cut: bool = False):
        ...
        metadata = {
            ...
            # Full transcript / spoken content of this clip
            "transcript_text": transcript_text,
            # Source video context
            "source_url": self.source_url,
            "source_video_title": self.source_video_info.get("title", ""),
            "source_video_description": self.source_video_info.get("description", "")[:500],
        ...
        # Save human-readable content summary as TXT
        self._save_content_txt(clip_dir, metadata, transcript_text)

        # Auto-upload to Google Drive if configured
        gdrive_cfg = self.gdrive_settings
        if gdrive_cfg.get("enabled") and gdrive_cfg.get("auto_upload"):
            upload_target = self.gdrive_session_folder_id or gdrive_cfg.get("folder_id", "")
            if upload_target:
                try:
                    from gdrive_uploader import GDriveUploader
                    uploader = GDriveUploader(log_callback=self.log)
                    if uploader.is_authenticated():
                        ...
                        self.log("  ⚠ Google Drive not authenticated — skipping upload")
                except Exception as e:
                    self.log(f"  ⚠ Google Drive upload failed: {e}")
    
    def _init_gdrive_session(self):
        """Create one Drive session folder per video batch (named after the YT title).

        Stores the folder ID in self.gdrive_session_folder_id so every clip upload
        goes into the same session subfolder instead of the raw root folder.
        """
        gdrive_cfg = self.gdrive_settings
        if not (gdrive_cfg.get("enabled") and gdrive_cfg.get("auto_upload")):
            return
        root_folder_id = gdrive_cfg.get("folder_id", "")
        ...
        try:
            ...
            if not uploader.is_authenticated():
                self.log("  ⚠ Google Drive not authenticated — uploads will be skipped")
                return
            yt_title = self.source_video_info.get("title", "") or "YT Clips"
            self.gdrive_session_folder_id = uploader.create_session_folder(root_folder_id, yt_title)
        except Exception as e:
            self.log(f"  ⚠ Could not create Drive session folder: {e}")
            self.gdrive_session_folder_id = None

    def _save_content_txt(self, clip_dir: Path, metadata: dict, transcript_text: str):
        """Save a human-readable content summary (content.txt) alongside the clip."""
        virality = metadata.get("virality_score", 0)
        if virality >= 7:
            virality_label = "🔥 Tinggi"
        elif virality >= 5:
            virality_label = "⚡ Sedang"
        else:
            virality_label = "💫 Rendah"

        lines = [
            "=" * 60,
            "YT SHORT CLIPPER — RINGKASAN KONTEN CLIP",
            "=" * 60,
            "",
            f"Judul Clip     : {metadata.get('title', '')}",
            f"Channel        : {metadata.get('channel_name', '')}",
            f"Sumber Video   : {metadata.get('source_video_title', '')}",
            f"URL            : {metadata.get('source_url', '')}",
            f"Dibuat         : {metadata.get('created_at', '')}",
            f"Clip           : {metadata.get('clip_index', 0)} / {metadata.get('total_clips', 0)}",
            "",
            "-" * 60,
            "INFO CLIP",
            "-" * 60,
            f"Timestamp      : {metadata.get('start_time', '')} → {metadata.get('end_time', '')}",
            f"Durasi         : {metadata.get('duration_seconds', 0):.0f} detik",
            f"Virality Score : {virality}/10  {virality_label}",
            "",
            "-" * 60,
            "HOOK TEXT",
            "-" * 60,
            ...
            "",
            "-" * 60,
            "DESKRIPSI (KENAPA VIRAL)",
            "-" * 60,
            metadata.get("description", ""),
            "",
            "-" * 60,
            "TRANSCRIPT / ISI PERCAKAPAN",
            "-" * 60,
            transcript_text if transcript_text else "(tidak tersedia)",
            "",
            "-" * 60,
            "DESKRIPSI VIDEO SUMBER",
            "-" * 60,
            metadata.get("source_video_description", ""),
            "",
            "=" * 60,
        ]

        txt_path = clip_dir / "content.txt"
...
"""
    ...
    def find_highlights_only(self, url: str, num_clips: int = 5,
        ...
        Returns:
            dict with keys:
                ...
                - 'clip_mode': The mode used to generate highlights
        """
        # Store source URL for metadata
        self.source_url = url
    ...
    def process_selected_highlights(self, url: str, selected_highlights: list, 
        ...
        # Load source video info from session_data.json if available
        session_data_file = session_dir / "session_data.json"
        if session_data_file.exists() and not self.source_video_info:
            try:
                with open(session_data_file, "r", encoding="utf-8") as f:
                    _sd = json.load(f)
                self.source_video_info = _sd.get("video_info", {})
            except Exception:
                pass
