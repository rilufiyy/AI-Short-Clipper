...
class YTShortClipperApp(ctk.CTk):
    ...
    def reset_home_page(self):
        ...
        # Reset clip mode to default
        self.clip_mode_var.set("Random Short Video")
    ...
    def create_home_page(self):
        ...
        # Clip Mode
        ctk.CTkLabel(left_col, text="Clip Mode", font=ctk.CTkFont(size=11, weight="bold"),
            anchor="w").pack(fill="x", pady=(8, 3))

        self.clip_mode_var = ctk.StringVar(value="Random Short Video")
    ...
    def load_config(self):
        # Try loading from new multi-provider structure first
    ...
    def _start_processing_validated(self):
        ...
        clip_mode_label = self.clip_mode_var.get()
        clip_mode = "continue_part" if clip_mode_label == "Continue Part Video" else "random"

        # NEW FLOW: Only download subtitle + find highlights (no video download)
        threading.Thread(target=self.run_find_highlights,
                        args=(url, num_clips, output_dir, model, subtitle_lang, clip_mode),
                        daemon=True).start()
    ...
    def run_find_highlights(self, url, num_clips, output_dir, model, subtitle_lang="id",
        ...
        # Store clip_mode so the Whisper fallback path can use it
        self.current_clip_mode = clip_mode
        try:
            ...
            try:
                ...
                result = core.find_highlights_only(url, num_clips, clip_mode=clip_mode)
        ...
        if result:
            ...
            threading.Thread(
                ...
                args=(core, snf_error.video_path, snf_error.video_info,
                      ...
                      getattr(self, 'current_clip_mode', 'random')),
                daemon=True
            ).start()
        else:
            self.on_error(
    ...
    def _run_whisper_transcription(self, core, video_path: str, video_info: dict,
                                    num_clips: int, session_dir: str, clip_mode: str = "random"):
        """Run Whisper transcription fallback in background thread."""
        try:
            result = core.find_highlights_with_transcription(
                video_path, video_info, num_clips, session_dir, clip_mode=clip_mode
            )
